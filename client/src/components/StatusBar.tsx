...
const StatusBar: React.FC<Props> = ({ projectId }) => {
  ...
  const pill = (label: string, state?: string) => (
    <span className={`px-2 py-1 rounded-xl text-xs ${state === 'ok' ? 'bg-green-700' : state === 'error' ? 'bg-red-700' : 'bg-[#f3f4f6] dark:bg-[#374151]'}`}>{label}: {state || '...'}</span>
  );

  return (
    <div className="fixed bottom-0 left-0 right-0 bg-[#ffffff] dark:bg-[#1f2937] text-[#4b5563] dark:text-[#d1d5db] p-2 text-xs flex justify-between items-center z-50 shadow-[0_-2px_10px_rgba(0,0,0,0.5)]">
      <div className="flex gap-4 items-center">
        <span className={`px-2 py-1 rounded-xl text-xs ${connected ? 'bg-green-700 text-[#111827] dark:text-[#f9fafb]' : 'bg-red-700 text-[#111827] dark:text-[#f9fafb]'}`}>
