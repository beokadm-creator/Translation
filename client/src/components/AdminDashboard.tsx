...
const AdminDashboard: React.FC = () => {
  ...
  const triggerRemaster = async () => {
      ...
      try {
          ...
          if (data.success) {
              if (data.count > 0) {
                  alert(`신규 자막 ${data.count}개에 대한 리마스터링이 완료되었습니다.`);
              } else {
                  alert("재검토 완료: 변경할 내용이 없거나 이미 최적화된 상태입니다.");
              }
          } else {
              alert("리마스터링 요청 실패");
          }
  ...
  const uploadChunks = async (chunks: Blob[]) => {
    ...
    try {
        ...
        fetch(url, { method: "POST", headers: { "Content-Type": "application/octet-stream", ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: buf }).catch(console.error);
        setStatus("streaming");
