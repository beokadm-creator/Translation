...
const LiveConsole: React.FC<Props> = ({ projectId, sourceLabel = "presenter" }) => {
  ...
  const lastOriginalTsRef = useRef<number>(0);
  const lastRefinedTsRef = useRef<number>(0);
  ...
  useEffect(() => {
    ...
    onValue(streamRef, (snap) => {
      ...
      const entries = Object.entries<any>(data).sort((a, b) => {
      ...
      // Append new originals
      entries.forEach(([, v]) => {
        const ot = Number(v?.original?.timestamp || 0);
        const rt = Number(v?.refined?.timestamp || 0);
        if (v?.original?.text && ot > lastOriginalTsRef.current) {
          setOriginalText((prev) => (prev ? prev + " " + v.original.text : v.original.text));
          lastOriginalTsRef.current = ot;
        }
        if (v?.refined?.text && rt > lastRefinedTsRef.current) {
          setRefinedText((prev) => (prev ? prev + " " + v.refined.text : v.refined.text));
          lastRefinedTsRef.current = rt;
        }
      });
    });
  ...
  return (
    <div className="space-y-4">
        ...
        <label className="flex items-center gap-2 text-sm">
          <input
            ...
            onChange={(e) => {
              ...
              if (micGainRef.current) micGainRef.current.gain.value = enabled ? (systemStream ? 0.2 : 1.0) : 0.0;
            }}
          />
          Mic Enable
        </label>
      ...
      <div className="bg-gray-800 rounded p-4">
        ...
        <div className="text-lg font-semibold break-words whitespace-pre-wrap">{originalText}</div>
      </div>
      <div className="bg-gray-800 rounded p-4">
        <div className="text-xs text-gray-400 mb-2">Refined</div>
        <div className="text-lg font-semibold break-words whitespace-pre-wrap">{refinedText}</div>
      </div>
    </div>
  );
};
